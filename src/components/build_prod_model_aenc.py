...
with mlflow.start_run() as run:
    ...
    FEATURES, _ = dp.get_features_and_targets(features_data, "all")
    BASE_MODELS = tuple(base_models)
    with open(args["zero_cols"], "r") as f:
        ZERO_COLS = [col for col in f.read().splitlines()]

    # SET PREDICT FUNCTION
    if ensembling_method == "simple":

        def predict(live_features: pd.DataFrame) -> pd.DataFrame:
            x = MinMaxScaler().fit_transform(live_features[FEATURES].to_numpy())
            latent_space = ENCODER(x)
            features = pd.DataFrame(latent_space)
            features.columns = [f"feat_{col}" for col in features.columns]
            features = features[sorted(features.columns)]
            features = (
                features.drop(columns=ZERO_COLS)
                .rank(pct=True)
